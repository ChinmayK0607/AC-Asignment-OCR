...
def process_image(image_bytes: bytes) -> str:
    ...
    try:
        ...
        raise e
async def ocr_moondream(image: Image.Image) -> str:
    try:
        enc_image = moondream_model.encode_image(image)
        description = moondream_model.answer_question(
            enc_image, "Extract the text from this image.", moondream_tokenizer
        )
        return description
    except Exception as e:
        logger.error(f"Error processing image with Moondream: {str(e)}")
        raise e

@app.post("/ocr_moondream")
async def ocr_moondream_route(file: UploadFile = File(...)):
    try:
        image_bytes = await file.read()
        image = Image.open(io.BytesIO(image_bytes))
        description = await ocr_moondream(image)
        return JSONResponse(content={"description": description})
    except Exception as e:
        logger.error(f"Error during Moondream OCR: {str(e)}")
